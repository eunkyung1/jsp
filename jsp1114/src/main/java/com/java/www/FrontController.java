...
public class FrontController extends HttpServlet {
	...
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("Controller에서 확인한 클릭한 파일 주소 :"+fileName);
		//이동주소
		String url = "";
		...
		if(fileName.equals("/login.do")) {
			...
			//단점 : sendRedirect - request,response 값 리셋하고 return을 해줌.
			//그래서 sendRedirect는 사용하지 않을 예정.
		}else if(fileName.equals("/write.do")) {
			url = "write.jsp";
		}else if(fileName.equals("/mwrite.do")) {
			url = "mwrite.jsp";
		}else if(fileName.equals("/doLoin.do")) {
			url = "Do_login";
		}
