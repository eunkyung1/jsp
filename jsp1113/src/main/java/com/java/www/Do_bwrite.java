...
public class Do_bwrite extends HttpServlet {
	...
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int size = 10*1024*1024; //50Mb = 50m*1024kb*1024b 
		//request,파일저장경로,크기,인코딩,같은이름변경정책 -> 1.jpg -> 1(1).jps->1(2).jpg
		...
		//while(files.hasMoreElements()) {
			...
			//bfile[i] = multi.getFilesystemName(file_arr[i]); //1.jpg 2.jgp 3.jpg
			//i++;
		//}
		...
		//	System.out.println("input Name:"+file_arr[i]); // input type의 name
		//	System.out.println("변경된 파일이름:"+bfile[i]); // 1(0).jpg
		//}
		
		//파일 1개일 때
		//System.out.println("input Name:"+file1); // input type의 name
		//System.out.println("실제파일이름:"+bfile); // 1(0).jpg
